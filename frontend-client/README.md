...
## DRCK variant (this repo)

To use the **DRCK** branding and preconfigured homeserver/Element Call:

1. Edit **`config/drck/config.json`** (or `element.io/drck/config.json`) and set your `default_server_name`, `default_server_config.m.homeserver.base_url`, and `element_call.url`.
2. Fetch the webapp with that config:  
   `yarn run fetch --cfgdir element.io/drck --noverify`
3. Run: `yarn run build:ts && yarn run build:res && yarn start`

To build installers with DRCK branding:  
`VARIANT_PATH=element.io/drck/build.json yarn run build`

For **Discord-style layout** (server list left, members right, "Servers" label), see [docs/DISCORD_UI_AND_CUSTOMIZATION.md](docs/DISCORD_UI_AND_CUSTOMIZATION.md) — those changes are made in Element Web, not in this desktop wrapper.
