# Enterprise Matrix Stack (k3s)

Production-ready example for deploying a **Matrix homeserver (Synapse)** with **Discordify module**, **Element Call**, **LiveKit SFU**, **Redis**, **PostgreSQL** (main + media + optional federated), and **TURN** on **Kubernetes / k3s**.
...
- **k8s/base/** – Kubernetes manifests for the full stack (namespace, secrets template, Postgres, Redis, Synapse, LiveKit, Element Call, TURN; optional federated Postgres).
- **scripts/generate_manifests.py** – Reads the config and generates Synapse `homeserver.yaml`, ConfigMap, and optional `.env` for secrets.
