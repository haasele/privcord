...
// Bundled English fallback so UI never shows raw i18n keys when i18n fetch fails or is delayed
import enFallback from "./i18n/strings/en_EN.json";
let bundledEnRegistered = false;
function registerBundledEnFallback(): void {
    if (bundledEnRegistered) return;
    registerTranslations("en", enFallback as Record<string, unknown>);
    bundledEnRegistered = true;
}
...
/** Max time to wait for i18n fetch/load; after this we use bundled English so the app still loads. */
const LANGUAGE_LOAD_TIMEOUT_MS = 3000;

export async function setLanguage(...preferredLangs: string[]): Promise<void> {
    registerBundledEnFallback();
    PlatformPeg.get()?.setLanguage(preferredLangs);

    const core = async (): Promise<void> => {
        let availableLanguages: Record<string, string>;
        try {
            availableLanguages = await getLangsJson();
        } catch (e) {
            logger.warn("Failed to load i18n/languages.json, using bundled English", e);
            setLocale("en");
            return;
        }
        ...
        if (!availableLanguages[chosenLanguage]) {
            setLocale("en");
        ...
        try {
            ...
            logger.warn("Failed to load language file, using bundled English", e);
            setLocale("en");
        }
        ...
        if (chosenLanguage !== "en" && availableLanguages["en"]) {
            try {
                ...
                // keep bundled en already registered
            }
        }
    ...
    const timeoutFallback = new Promise<void>((resolve) => {
        setTimeout(() => {
            logger.warn("Language load timed out after " + LANGUAGE_LOAD_TIMEOUT_MS + "ms, using bundled English");
            setLocale("en");
            void SettingsStore.setValue("language", null, SettingLevel.DEVICE, "en")
                .then(() => registerCustomTranslations())
                .then(resolve);
        }, LANGUAGE_LOAD_TIMEOUT_MS);
    });

    return Promise.race([core(), timeoutFallback]);
}
