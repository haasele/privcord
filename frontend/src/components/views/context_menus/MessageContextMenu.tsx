...
import { canSendDiscordifyModerationInSpace } from "../../../utils/discordify";
import { showBanUserDialog } from "../dialogs/BanUserDialog";
import { showTimeoutUserDialog } from "../dialogs/TimeoutUserDialog";
...
export default class MessageContextMenu extends React.Component<IProps, IState> {
    ...
    public render(): React.ReactNode {
        ...
        // Discordify: Ban / Timeout in server (space) – only if room has parent space and user can moderate
        const senderId = mxEvent.getSender();
        const roomId = mxEvent.getRoomId();
        const parentSpaceId = roomId ? SpaceStore.instance.getCanonicalParent(roomId) : undefined;
        const parentSpace = parentSpaceId ? cli.getRoom(parentSpaceId) : undefined;
        const canModerate = parentSpace && senderId && senderId !== me && canSendDiscordifyModerationInSpace(cli, parentSpace);
        const room = roomId ? cli.getRoom(roomId) : undefined;
        const senderMember = room && senderId ? room.getMember(senderId) : undefined;
        const senderDisplayName = senderMember?.rawDisplayName ?? senderId ?? "";

        let banUserButton: JSX.Element | undefined;
        let timeoutUserButton: JSX.Element | undefined;
        if (canModerate && parentSpace) {
            banUserButton = (
                <IconizedContextMenuOption
                    icon={<ErrorSolidIcon />}
                    label={_t("action|ban")}
                    onClick={() => {
                        showBanUserDialog(parentSpace, senderId!, senderDisplayName);
                        this.closeMenu();
                    }}
                />
            );
            timeoutUserButton = (
                <IconizedContextMenuOption
                    icon={<RestartIcon />}
                    label={_t("discordify|timeout_action")}
                    onClick={() => {
                        showTimeoutUserDialog(parentSpace, senderId!, senderDisplayName);
                        this.closeMenu();
                    }}
                />
            );
