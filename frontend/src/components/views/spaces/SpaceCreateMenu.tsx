...
import { showCreateServerDialog } from "../dialogs/CreateServerDialog";
import { showFederationDirectoryDialog } from "../dialogs/FederationDirectoryDialog";
    ...
    if (visibility === null) {
        body = (
            <React.Fragment>
                ...
                {SdkConfig.get().server_creation_api_url && (
                    <AccessibleButton
                        kind="primary_outline"
                        className="mx_SpaceCreateMenu_createServer"
                        onClick={() => {
                            onFinished();
                            showCreateServerDialog();
                        }}
                    >
                        {_t("create_server|title")}
                    </AccessibleButton>
                )}
                {SdkConfig.get().federation_directory_url && (
                    <AccessibleButton
                        kind="primary_outline"
                        className="mx_SpaceCreateMenu_discoverServers"
                        onClick={() => {
                            onFinished();
                            showFederationDirectoryDialog();
                        }}
                    >
                        {_t("federation_directory|title")}
                    </AccessibleButton>
                )}
            </React.Fragment>
        );
